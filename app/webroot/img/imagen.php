...
$carpetaImagen = $_GET['options']; // carpeta de la imagen
$nombreImagen = $_GET['img']; // nombre de la imagen
$wd = $_GET['wd']; // ancho deseado para la imagen
$hd = $_GET['hd']; // alto deseado para la imagen
$fullName = $carpetaImagen.'/'.$nombreImagen;
list($ow, $oh, $xmime) = getimagesize($fullName);
$imageSize = filesize($fullName);
$mime = '';
if($xmime == 1) { $mime = 'image/gif'; $tipo = "gif"; }
if($xmime == 2) { $mime = 'image/jpg'; $tipo = "jpg"; }
if($xmime == 3) { $mime = 'image/png'; $tipo = "png"; }

$f = fopen($fullName,"r") or die("Invalid file stream.");
$imageData = fread($f, $imageSize);
fclose($f);

$r = new ImageResizer();
$newImage = $r->resize($imageData, $wd, $hd, 70, 'jpg', $ow, $oh);
header('Content-type: '.$mime);
echo $newImage;
?>
