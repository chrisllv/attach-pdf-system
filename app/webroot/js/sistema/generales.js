var LinkPJ = "";//RUTA DEL PROYECTO SIN EL / AL FINAL

function ver_documento(url_busqueda,ID){
	new Ajax.Updater(ID ,LinkPJ+url_busqueda, {asynchronous:true, evalScripts:true, onLoading:function(request){Element.show('mini_loading');}, onComplete:function(request){Element.hide('mini_loading')}, requestHeaders:['X-Update', ID]})
}


function ver_documento2(url_busqueda,ID){
	new Ajax.Updater2(ID ,LinkPJ+url_busqueda, {asynchronous:true, evalScripts:true, onLoading:function(request){Element.show('mini_loading');}, onComplete:function(request){Element.hide('mini_loading')}, requestHeaders:['X-Update', ID]})
}
...
function cargar_contenido(url_cargar,id_cargar){
     new Ajax.Updater(id_cargar,LinkPJ+url_cargar, {asynchronous:true, evalScripts:true,onLoading:function(request){Element.show('mini_loading');}, onComplete:function(request){Element.hide('mini_loading')}, requestHeaders:['X-Update', id_cargar]});
...
function input_remoto(url_cargar,id_cargar,parametro_extra){
   url_cargar = LinkPJ+url_cargar+'/'+parametro_extra;
...
function ventanaGoogleMap(){
  ...
  var ventana = window.open(LinkPJ+'/adm_mapas/buscar_puntos',"GOOGLE_MAPA_FIND2","width=590,height=420,fullscreen=0,scrollbars=no,resizable=no,location=no,status=no,menubar=no,toolbar=no,titlebar=no");
}
