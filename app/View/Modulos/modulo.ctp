...
if(isset($NIVEL1)){
	...
	foreach($NIVEL1 as $n1){
		$n1['Zmenu']['url'] = $this->Html->urlink(ur1($n1['Zmenu']['url']));
		...
		foreach($NIVEL2[$n1['Zmenu']['id']] as $n2){
			$n2['Zmenu']['url'] = $this->Html->urlink(ur1($n2['Zmenu']['url']));
			...
			foreach($NIVEL3[$n2['Zmenu']['id']] as $n3){
				$n3['Zmenu']['url'] = $this->Html->urlink(ur1($n3['Zmenu']['url']));
				...
				foreach($NIVEL4[$n3['Zmenu']['id']] as $n4){
					$n4['Zmenu']['url'] = $this->Html->urlink(ur1($n4['Zmenu']['url']));
                    ...
                    foreach($NIVEL5[$n4['Zmenu']['id']] as $n5){
						$n5['Zmenu']['url'] = $this->Html->urlink(ur1($n5['Zmenu']['url']));
	                    ...
	                    foreach($NIVEL6[$n5['Zmenu']['id']] as $n6){
							$n6['Zmenu']['url'] = $this->Html->urlink(ur1($n6['Zmenu']['url']));
		                    ...
		                    foreach($NIVEL7[$n6['Zmenu']['id']] as $n7){
								$n7['Zmenu']['url'] = $this->Html->urlink(ur1($n7['Zmenu']['url']));
			                    ...
			                    foreach($NIVEL8[$n7['Zmenu']['id']] as $n8){
									$n8['Zmenu']['url'] = $this->Html->urlink(ur1($n8['Zmenu']['url']));
...
?>
	...
	if(isset($modulos)){$i=1;
	    foreach($modulos as $mod){
	    	extract($mod['Modulo']);
	    	echo "\n\t\t[null,'".mascara($i,2)." - ".$denominacion."','".$this->Html->url("/modulos/modulo/$id/msj")."','no_ajax',''],";
	     $i++;
	     }//fin foreach
	}//fin if
	?>
	['<img src="<?= $url ?>img/input_pass.png" border="0" title="Cambiar Clave" width="18" height="16">','Cambiar Clave', '<?= $this->Html->url("/conf_usuarios/cambiar_clave") ?>','principal',''],
	['','Regresar a Módulos', '<?= $this->Html->url("/modulos/view_link/") ?>','no_ajax','']
    ],
	['<img src="<?= $url ?>img/stock_exit.png" border="0" title="Cerrar Session" width="24" height="24">','', '<?= $this->Html->url("/entrada/salir") ?>','no_ajax','']
];
